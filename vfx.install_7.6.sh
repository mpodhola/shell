...
INST_LIST="$INSPATH/vfx.install76.txt"
...
case $RELEASE in
  7.6.1810)
    ...
    echo "[srv-repos01.centos.std.7.6.1810.repo]" > $CENTOS
    echo "name=SRV-REPOS01.UPP.CZ - CENTOS STD 7.6.1810 REPOSITORY" >> $CENTOS
    echo "baseurl=http://srv-repos01.upp.cz/repos/centos/7.6.1810/" >> $CENTOS
    ...
    echo "[srv-repos01.centos.adds.7.6.1810.repo]" > $VFX
    echo "name=SRV-REPOS01.UPP.CZ - CENTOS ADS 7.6.1810 REPOSITORY" >> $VFX
    echo "baseurl=http://srv-repos01.upp.cz/repos/centos/7.6.1810.add" >> $VFX
    echo "enabled=1" >> $VFX
    echo "gpgcheck=0" >> $VFX
       ;;
  * )
  echo "nemate spravnou verzi Centosu bud 7.6 "
  exit 1
  ;;
  esac
...
# --- Option processing --------------------------------------------
privilidge
while getopts ":vhgn:N:cRsbfKSI:Utwlmxork" optname
  do
    case "$optname" in
      "v")
        version
